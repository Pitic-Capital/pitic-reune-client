...
import { getReclamaciones, getReclamacionesTotal, deleteReclamacion } from "../../api/reune.client";
...
const ComplaintsTable = () => {
   ...
   const fetchData = useCallback(async () => {
      ...
      try {
         ...
         const { data: totalData } = await getReclamacionesTotal(token);
         ...
         const requests = Array.from({ length: totalPages }, (_, i) => getReclamaciones(token, i + 1));
         ...
         setError("Error al consultar reclamaciones: " + msg);
         setConsultData([]);
      }
   }, []);
   ...
   const handleConfirmDelete = async () => {
      ...
      setIsDeleting(true);
      try {
         await deleteReclamacion(token!, deleteItem.RecFolioAtencion);
         showSnackbar("Reclamación eliminada exitosamente", "success");
         fetchData();
      } catch (err: any) {
   ...
   return (
      <Stack spacing={2}>
         ...
         <ConfirmDeleteDialog
            open={isDialogOpen}
            folio={deleteItem?.RecFolioAtencion}
            loading={isDeleting}
